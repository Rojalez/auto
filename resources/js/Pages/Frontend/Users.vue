    ...
    <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-x-auto shadow-xl sm:rounded-lg px-4 py-4">
                    ...
                    <button @click="openModal()" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded my-3">Добавить пользователя</button>
                    <div class='overflow-x-auto rounded-lg'>
                        <table class="w-full">
                        <thead >
                            <tr class="bg-gray-700 text-white border text-left">
                                <th class="border px-4 py-2">Имя</th>
                                <th class="border px-4 py-2">Номер</th>
                                <th class="border px-4 py-2 text-center">Скидка</th>
                        ...
                        <tbody v-for="row in data" v-bind:key="row.id" class="border">
                            <tr>
                                ...
                                <td class="border px-4 py-2 text-center">{{ row.procent }}</td>
                                <td class="border px-4 py-2">
                                    <div class='flex flex-row flex-wrap'> <button @click="edit(row)" class="flex-auto bg-yellow-500 hover:bg-indigo-600 text-white font-bold py-2 px-4 rounded mx-2 my-1">Изменить</button>
                          ...
                          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="">
                                  ...
                                  <div class="mb-4">
                                      ...
                                      <input type="number" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="exampleFormControlInput1" placeholder="Номер" v-model="form.phone">
                                  </div>
                                  <div class="mb-4" >
                                      <label for="exampleFormControlInput5" class="block text-gray-700 text-sm font-bold mb-2">Скидка:</label>
                                    <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="exampleFormControlInput5" v-model="form.procent">
                                      <option value=0>Без скидок</option>
                                      <option v-for="skidka in skidki" :key="skidka" :value="skidka.procent">{{skidka.name}}</option>
                                    </select>
                                  </div>
                                  <div class="mb-4" >
                                      <label for="exampleFormControlInput1" class="block text-gray-700 text-sm font-bold mb-2">Админ:</label>
                                      <input type="checkbox" class="shadow appearance-none border rounded py-2 px-2 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="exampleFormControlInput1" placeholder="Статус" v-model="form.admin">
                                  </div>
                                  <div class="mb-4">
                                      <label for="exampleFormControlInput1" class="block text-gray-700 text-sm font-bold mb-2">Пароль:</label>
                                      <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="exampleFormControlInput1" placeholder="Пароль" v-model="form.password">
                                  </div>
