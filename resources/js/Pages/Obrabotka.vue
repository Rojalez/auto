<template>
    <app-layout>
        ...
        <div class="py-12">
            ...
            <div class='mb-4 md:mx-0 mx-4'><span class='bg-yellow-400 hover:bg-yellow-500 rounded-md px-4 py-2 text-white cursor-pointer' @click="filter = !filter">Фильтр</span></div>
            <div class='flex flex-wrap justify-items-between mx-auto w-full items-center px-2 py-2 relative' v-if='filter'>
                ...
                <div class='flex-auto p-2 w-full lg:w-auto'><button @click="search" class='bg-yellow-400 hover:bg-yellow-500 px-3 py-2 rounded-md text-white w-full lg:w-auto'>Поиск</button></div>
            </div>

                <div class="bg-transparent w-full sm:rounded-lg">
                    <div class='flex flex-col px-4 lg:px-0'>
                        <div
                            class="lg:grid hidden  grid-cols-10 gap-1 mt-1 md:mt-4 py-2 rounded-t-lg bg-gray-700 text-white">
                        ...
                        <div class='lg:grid lg:grid-cols-10 flex flex-col gap-1 border py-2 border bg-white mb-2 hover:bg-gray-50 lg:rounded-none rounded-lg' v-for="part in data" :key='part'>
                                ...
                                <div class='lg:border-r flex flex-col sm:text-sm md:text-md px-1 py-2 ml-2 md:ml-0 md:border-b-0 border-b-2 break-words lg:mx-0 mx-2'>
                                    <span class='lg:hidden inline-block font-semibold'>Статус:</span>
                                    <span class='animate-pulse text-yellow-500 md:ml-0 ml-1'>{{part.status}}</span>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            search() {
                this.$inertia.get('/dashboard/obrabotka', this.form2)
            }
        }
    }
</script>